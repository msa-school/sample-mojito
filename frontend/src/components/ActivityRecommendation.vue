<template>
    ...
    <v-card style="width:300px;" outlined>
        ...
        <v-card-title v-if="value._links">
            ActivityRecommendation # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
        ...
        <v-card-text>
            <div>
                <String label="Rating" v-model="value.rating" :editMode="editMode"/>
            </div>
            <div>
    ...
    export default {
        ...
        created(){
            ...
            if(typeof this.value === 'object') {
                if(!('rating' in this.value)) {
                    this.value.rating = null;
                }
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/activityRecommendations'), this.value)
                        } else {
